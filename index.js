...
const zooAnimals = [
  ...
  */
  ...
  function lowerCaseNames(arr){
    const lowerCase = [];
    //using .map() to convert the arr data into just lower case animal_name properties;
    arr.map(animal=>lowerCase.push(animal.animal_name.toLowerCase()))
    return lowerCase;
  }
  ...
  */
  ...
  function lowPopulationAnimals(arr){
    //using .filter() to filter out all animals with population more than 5
    const filtered = arr.filter(animal=>animal.population<5);
    return filtered;
  }
  ...
  */
  ...
  function USApop(arr){
    //using reduce to return the total population
    return arr.reduce((accum,current)=>accum+=current.population,0);
  }
...
function CuboidMaker(obj){
  //instantiating the object with the length with and height properties of the passed through object
...
CuboidMaker.prototype.volume = function(){
  //returns the formula for volume length * width * height ;
...
CuboidMaker.prototype.surfaceArea = function(){
  //returns the formula for surface area 2 * (length * width + length * height + width * height);
